    ...
    return (
        <Container>
            <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }} xs={6} md={12}>
                <CustomText text={'WHY CHOOSE'} fontSize={48} marginRight={1.5} fontWeight={600} />
                <CustomText text={'OPEN SOURCE?'} fontSize={48} color={'#26CB99'} fontWeight={600} />
            </Box>
            <CustomText
            ...
            <Grid container display={'flex'} justifyContent="center" rowSpacing={5}>
                {data.map(val => (
                    <Grid item xs={6} md={4} key={val.id}>
                        <Image src={val.image} width={80} height={80} style={{ marginBottom: 15 }} />
                        <CustomText text={val.title} fontSize={28} whiteSpace={'pre-line'} fontWeight={600} marginBottom={1} />
